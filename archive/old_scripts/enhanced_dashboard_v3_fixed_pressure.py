#!/usr/bin/env python3
"""
Enhanced Bali et al. 2012 Model Dashboard v3 - Fixed Pressure

Complete interactive implementation with fixed mantle compositions,
multi-salinity analysis with individual control, fO2 variation, and modal mineralogy exploration.
Pressure fixed at 26100 bar as requested.

Usage: streamlit run enhanced_dashboard_v3_fixed_pressure.py
"""
...
# ========== PAGE CONFIGURATION ==========
st.set_page_config(
    page_title="Bali et al. 2012 Enhanced Model v3",
    page_icon="🧪",
    layout="wide",
...
# ========== DASHBOARD TITLE ==========
st.title("🧪 Bali et al. 2012 Enhanced Interactive Model v3")
st.markdown("**Complete Trace Element Partitioning Analysis: Fixed Mantle Compositions • Selectable Salinities • fO₂ • Modal Variations**")
st.markdown("---")
...
# ========== SALINITY SELECTION ==========
st.sidebar.subheader("🧂 Salinity Selection")
st.sidebar.markdown("*Select salinities to display:*")

# Individual checkboxes for each salinity
use_sal_0001 = st.sidebar.checkbox("0.001% NaCl", value=True)
use_sal_5 = st.sidebar.checkbox("5% NaCl", value=True)
use_sal_10 = st.sidebar.checkbox("10% NaCl", value=True)
use_sal_15 = st.sidebar.checkbox("15% NaCl", value=True)
use_sal_20 = st.sidebar.checkbox("20% NaCl", value=True)

# Build selected salinities list
selected_salinities = []
salinity_mapping = {
    use_sal_0001: 0.001,
    use_sal_5: 5.0,
...
for checkbox, salinity in salinity_mapping.items():
    if checkbox:
        selected_salinities.append(salinity)

# If no salinity selected, default to 10%
if not selected_salinities:
    selected_salinities = [10.0]
    st.sidebar.warning("⚠️ No salinity selected - defaulting to 10%")

# Current salinity focus (for detailed display)
available_options = [sal for sal in [0.001, 5.0, 10.0, 15.0, 20.0] if sal in selected_salinities]
salinity_wt_pct = st.sidebar.selectbox("Focus Salinity for Details", available_options, 
                                      index=0 if available_options else 0)

st.sidebar.markdown(f"**Selected Salinities:** {len(selected_salinities)}")
...
current_result = next((r for r in all_composition_results[primary_composition] if r['salinity_wt'] == salinity_wt_pct), 
                     all_composition_results[primary_composition][0] if all_composition_results[primary_composition] else None)
...
st.subheader("📈 Bali et al. 2012 Model: Mo/Ce vs U/Th (Selected Salinities & Compositions)")

# Create the signature Mo/Ce vs U/Th plot with selected salinity lines for all compositions
fig_model = go.Figure()

# Colors for different salinities and compositions - map to specific salinities
salinity_color_map = {
    0.001: '#1f77b4',  # Blue
    5.0: '#ff7f0e',    # Orange
    10.0: '#2ca02c',   # Green
    ...
    for result in results:
        if np.isfinite(result['mo_ce_ratio']) and np.isfinite(result['u_th_ratio']):
            ...
            fig_model.add_trace(go.Scatter(
                ...
                y=[result['mo_ce_ratio']],
                mode='markers',
                name=f"{comp_style['name']}: {salinity_name_map[salinity]} NaCl",
                marker=dict(size=14, color=salinity_color_map[salinity], 
...
if current_result and np.isfinite(current_result['mo_ce_ratio']) and np.isfinite(current_result['u_th_ratio']):
    fig_model.add_trace(go.Scatter(
        x=[current_result['u_th_ratio']],
        y=[current_result['mo_ce_ratio']],
        mode='markers',
        name=f'FOCUS: {salinity_name_map[salinity_wt_pct]} NaCl ({composition_styles[primary_composition]["name"]})',
...
fig_model.update_layout(
    title=f"Model Conditions: T={temperature_c}°C, P={pressure_bar/1000:.1f} kbar (FIXED), ΔFMQ={dfmq:.1f}<br>" +
          f"Modal: CPX={mode_cpx_norm:.2f}, GRT={mode_grt_norm:.2f}, RUT={mode_rut_norm:.2f}<br>" +
          f"Selected: {', '.join([composition_styles[comp]['name'] for comp in selected_compositions])}, Salinities: {len(selected_salinities)}",
    xaxis_title="U/Th",
    yaxis_title="Mo/Ce", 
    xaxis_type="log",
    yaxis_type="log",
...
with col2:
    ...
    if current_result:
        st.write(f"**Focus: {composition_styles[primary_composition]['name']} at {salinity_name_map[salinity_wt_pct]} NaCl**")
        ...
        st.warning("⚠️ No data for current focus salinity")
...
# ========== DATA TABLE ==========
st.markdown("---")
st.subheader("📋 Complete Results Table (Selected Compositions & Salinities)")
...
st.markdown("**Dashboard v3:** Enhanced implementation with selectable salinities • Fixed axis ranges • DM & PM compositions only • **FIXED PRESSURE: 26.1 kbar**")
st.markdown("**Features:** Individual salinity control • Fixed Mo/Ce vs U/Th axis ranges • Real-time parameter variation")
