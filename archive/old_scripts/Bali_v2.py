...
def main():
    ...
    if args.salinity_wt is not None:
        ...
        if 1 <= sel_sal_i <= len(sal_options):
            ...
            print(f"Invalid salinity-index {sel_sal_i}, defaulting to option 1.")
            sel_wts = [sal_options[0]]
    else:
        sel_sal = input(f"Choose salinity option (1-{len(sal_options)}): ")
        try:
            sel_sal_i = int(sel_sal)
        except Exception:
            print(f"Invalid salinity selection {sel_sal!r}, defaulting to option 1.")
            sel_sal_i = 1
    ...
    with pd.option_context("display.max_columns", None, "display.width", 200):
        ...
        try:
            ...
            for sal, mix_df in all_mixes:
                ...
                # plot and annotate markers only for the requested fractions
                for f in label_fs:
                    ...
                    if not (math.isfinite(sx) and math.isfinite(sy)):
                        continue
                    # draw marker
                    ax.plot(sx, sy, marker='X', color='k', markersize=6, markeredgewidth=1)
                    # annotate with percentage label
                    pct = label_percent.get(f, f"{int(round(float(f)*100))}%")
                    # slight offset for text (multiplicative for log scale)
                    ax.text(sx * 1.03, sy * 1.03, pct, fontsize=8, color='k', horizontalalignment='left', verticalalignment='bottom')
                ...
                # (arrow removed; no arrow drawn)

                # (removed salinity inline label — only percentage labels for selected f values are shown)
                try:
                    pass
                except Exception:
                    pass
            ...
            ax.set_xlabel('U/Th')
            ax.set_ylabel('Mo/Ce')
            ax.set_title('DM-F Mixing — Mo/Ce vs U/Th')
            ...
            print('DEBUG: plotting failed (matplotlib may be unavailable or running headless).')
